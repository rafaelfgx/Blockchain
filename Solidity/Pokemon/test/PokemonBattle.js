...
describe("PokemonBattle", function () {
    ...
    async function mintCharmander(contract, signer) {
        const pokemon = { id: 0, name: "Charmander", hp: 100, attack: 100, defense: 100, speed: 100, specialAtack: 100, specialDefense: 100, element: 1, uri: "charmander.jpg", level: 1, experience: 0 };
        const evolutions = [{ name: "Charmeleon", uri: "charmeleon.jpg", level: 2 }, { name: "Charizard", uri: "charizard.jpg", level: 3 }];
        await contract.mint(signer.address, pokemon, evolutions);
    }

    async function mintSquirtle(contract, signer) {
        const pokemon = { id: 0, name: "Squirtle", hp: 50, attack: 50, defense: 50, speed: 50, specialAtack: 50, specialDefense: 50, element: 2, uri: "squirtle.jpg", level: 1, experience: 0 };
        const evolutions = [{ name: "Wartortle", uri: "wartortle.jpg", level: 2 }, { name: "Blastoise", uri: "blastoise.jpg", level: 3 }];
        await contract.mint(signer.address, pokemon, evolutions);
    }

    async function verify(contract, pokemon, name, level, uri) {
        expect(pokemon.name).to.equal(name);
        expect(pokemon.level).to.equal(level);
        expect(pokemon.uri).to.equal(uri);
        expect(await contract.tokenURI(pokemon.id)).to.contains(uri);
    }
    ...
    it("List", async function () {
        ...
        verify(contract, pokemons[0], "Charmander", 1, "charmander.jpg");
        verify(contract, pokemons[1], "Squirtle", 1, "squirtle.jpg");
    });
    ...
    it("Get", async function () {
        ...
        verify(contract, pokemon, "Charmander", 1, "charmander.jpg");
    });
    ...
    it("Battle -> Reverted", async function () {
        ...
        await expect(contract.connect(owner).battle(1, 2)).to.be.revertedWith("You must own at least one of the Pokemon to initiate a battle!");
    });
    ...
    it("Battle", async function () {
        ...
        verify(contract, charmeleon, "Charmeleon", 2, "charmeleon.jpg");
        verify(contract, squirtle, "Squirtle", 1, "squirtle.jpg");
    });
});
